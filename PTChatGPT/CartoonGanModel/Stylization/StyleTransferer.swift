...
class StyleTransferer {
    ...
    static func newCPUStyleTransferer(
        completion: @escaping ((Result_TF<StyleTransferer>) -> Void) ) -> () {
        return StyleTransferer.newInstance(transferModel: Constants.Int8.transferModel, predictModel: Constants.Int8.predictModel, useMetalDelegate: false, completion: completion)
    }

    /// Create a new Style Transferer instance.
    static func newInstance(transferModel: String, predictModel: String, useMetalDelegate: Bool, completion: @escaping ((Result_TF<StyleTransferer>) -> Void)) {
        ...
        tfLiteQueue.async {
            ...
            do {
                ...
                let predictInterpreter = try Interpreter( modelPath: predictModelPath, options: createOptions(), delegates: createDelegates() )
                let transferInterpreter = try Interpreter( modelPath: transferModelPath, options: createOptions(), delegates: createDelegates() )
                ...
                let styleTransferer = StyleTransferer( tfLiteQueue: tfLiteQueue, predictInterpreter: predictInterpreter, transferInterpreter: transferInterpreter )
                PTGCDManager.gcdMain {
                    completion(.success(styleTransferer))
                }
            } catch let error {
                PTNSLogConsole("Failed to create the interpreter with error: \(error.localizedDescription)")
                PTGCDManager.gcdMain {
    ...
    fileprivate init( tfLiteQueue: DispatchQueue, predictInterpreter: Interpreter, transferInterpreter: Interpreter ) {
    ...
    func runStyleTransfer(style styleImage: UIImage, image: UIImage, completion: @escaping ((Result_TF<StyleTransferResult>) -> Void)) {
        tfLiteQueue.async {
            ...
            do {
                ...
                guard let styleRGBData = styleImage.scaledData( with: Constants.styleImageSize, isQuantized: false ) else {
                    PTGCDManager.gcdMain {
                        completion(.error(StyleTransferError.invalidImage))
                    }
                    PTNSLogConsole("Failed to convert the style image buffer to RGB data.")
                    return
                }

                guard let inputRGBData = image.scaledData( with: Constants.inputImageSize, isQuantized: false ) else {
                    PTGCDManager.gcdMain {
                        completion(.error(StyleTransferError.invalidImage))
                    }
                    PTNSLogConsole("Failed to convert the input image buffer to RGB data.")
                    return
                }
                ...
                PTNSLogConsole("Failed to invoke the interpreter with error: \(error.localizedDescription)")
                PTGCDManager.gcdMain {
            ...
            postprocessingTime = timeSinceStart() - stylePredictTime - styleTransferTime - preprocessingTime

            // Return the result.
            DispatchQueue.main.async {
                completion( .success( StyleTransferResult( resultImage: outputImage, preprocessingTime: preprocessingTime, stylePredictTime: stylePredictTime, styleTransferTime: styleTransferTime, postprocessingTime: postprocessingTime )
                    )
                )
            }
        }
